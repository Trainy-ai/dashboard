...
const fetchData = async ( {setPlotInput, rank} ) => {
    const response = await fetch(`http://localhost:5000/idle_time?` + new URLSearchParams({rank}), {
...
const fetchNumRanks = async ( {setNumRanks} ) => {
    const response = await fetch(`http://localhost:5000/num_ranks`, {
        headers: { accept: "application/json" },
    })
    var out = await response.json();
...
function TemporalView() {
    ...
    useEffect(() => {
        ...
        const fetchData = async ( {setPlotInput, rank, visualizePct} ) => {
            const response = await fetch(`http://localhost:5000/idle_time?` + new URLSearchParams({rank, visualizePct}), {
                headers: { accept: "application/json" },
            })
